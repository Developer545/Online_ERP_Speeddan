// Notas de Débito Electrónica (DTE-06)
// Aumenta el valor de un DTE previamente emitido
...
import {
  PlusOutlined, ReloadOutlined, FilePdfOutlined
...
export default function NotaDebitoPage() {
  ...
  const cargar = useCallback(async (p = page) => {
    setLoading(true)
    try {
      const res = await window.billing.listar({ tipoDte: '06', page: p, pageSize })
      setNotas(res.facturas)
      setTotal(res.total)
    } catch {
      message.error('Error al cargar notas de débito')
    } finally {
      setLoading(false)
    }
  }, [page, pageSize])
  ...
  return (
    <>
      <Row gutter={[12, 12]} style={{ marginBottom: 16 }}>
        <Col xs={12} sm={8}>
          <Card size="small">
            <Statistic title="Total Notas de Débito" value={total} />
          </Card>
        </Col>
        <Col xs={12} sm={8}>
          <Card size="small">
      ...
      <Card
        title={
          <Space>
            <Title level={5} style={{ margin: 0 }}>Notas de Débito Electrónica</Title>
            <Tag color="red">DTE-06</Tag>
          </Space>
        }
        extra={
          <Space>
            <Tooltip title="Actualizar"><Button icon={<ReloadOutlined />} onClick={() => cargar(1)} loading={loading} /></Tooltip>
            <Button type="primary" danger icon={<PlusOutlined />} onClick={() => setModalOpen(true)}>
      ...
      <Modal
        title={<Space><span>Nueva Nota de Débito</span><Tag color="red">DTE-06</Tag></Space>}
        open={modalOpen}
        onCancel={resetModal}
        footer={null}
        width={780}
      ...
      >
        {resultado ? (
          resultado.ok ? (
            <Result
              status="success"
              title="Nota de Débito emitida exitosamente"
              subTitle={
            ...
            <Result
              status="error"
              title="Error al emitir la nota de débito"
          ...
          <Form form={form} layout="vertical" size="middle">
            ...
            {dteSeleccionado && (
              <div style={{ background: '#fff1f0', border: '1px solid #ffa39e', borderRadius: 6, padding: '8px 12px', marginBottom: 12 }}>
                <Space>
                  <Tag color="red">{dteSeleccionado.tipoDte === '01' ? 'FAC' : 'CCF'}</Tag>
            ...
            <Divider orientation="left" plain>Ítems a debitar</Divider>
                    ...
                    return (
                      <tr key={item.key} style={{ borderBottom: '1px solid #f0f0f0' }}>
                        <td style={{ padding: '4px 8px' }}>
                          <Input size="small" value={item.descripcion} placeholder="Concepto a debitar"
            ...
            {items.length > 0 && (
              <div style={{ textAlign: 'right', background: '#fafafa', padding: '8px 12px', borderRadius: 6, marginBottom: 12, fontSize: 12 }}>
                <Space direction="vertical" size={2} style={{ textAlign: 'right' }}>
                  ...
                  <Text style={{ fontSize: 14 }}>Total: <Text strong style={{ color: '#ff4d4f', fontSize: 14 }}>{formatCurrency(totalItems)}</Text></Text>
                </Space>
              </div>
            )}
